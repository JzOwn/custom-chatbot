...
export function Chat({ assistantId, threadId }: { assistantId: number; threadId: number }) {
	const { messages, loadMessages, sendMessage, streaming, error,loading } = useAppStore()
	...
	return (
		<div className="flex flex-col h-full max-h-[calc(100vh-50px)]">
			<div ref={listRef} className="flex-1 overflow-y-auto space-y-3 pr-2 pb-3">
				{items.map(m => (
					<div key={`${m.id}-${m.created_at}`} className={`p-3 rounded relative transition-colors ${
						m.role === 'user' 
						...
						<div className="text-xs text-gray-500 dark:text-gray-400 mb-1 flex items-center gap-2">
							<span className={`border px-1 py-0.5 rounded-md transition-colors ${
								m.role == 'user' 
			...
			<div className="sticky bottom-1 bg-white dark:bg-gray-800 pt-2 transition-colors">
				<form onSubmit={onSubmit} className="flex gap-2">
					...
					/>
					<button className="bg-blue-600 dark:bg-blue-500 text-white px-4 py-2 rounded disabled:opacity-50 hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors" disabled={!input.trim() || streaming}>
						Send
					</button>
				</form>
			</div>
