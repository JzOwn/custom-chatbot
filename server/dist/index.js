...
app.post('/api/assistant', async (c) => {
    try {
        ...
        const result = await query('INSERT INTO assistants (open_ai_assistant_id, name, system_prompt) VALUES ($1, $2, $3) RETURNING id', [created.id, name, instructions]);
...
app.put('/api/assistant/:id', async (c) => {
    try {
        ...
        const existing = await query('SELECT open_ai_assistant_id FROM assistants WHERE id=$1', [id]);
...
app.get('/api/assistant', async (c) => {
    const res = await query('SELECT id, open_ai_assistant_id, name, system_prompt, created_at FROM assistants ORDER BY created_at DESC');
...
app.post('/api/thread/:assistantId', async (c) => {
    try {
        ...
        const assistantRes = await query('SELECT open_ai_assistant_id FROM assistants WHERE id=$1', [assistantId]);
        ...
        const thrRes = await query('INSERT INTO threads (openai_threadid, thread_title, assistant_id) VALUES ($1, $2, $3) RETURNING id, openai_threadid, thread_title, assistant_id', [oaThread.id, title, assistantId]);
...
app.post('/api/chat/:assistantId', async (c) => {
    try {
        ...
        if (!threadId || !content)
            return c.json({ error: 'threadId and content are required' }, 400);
        const assistantRow = await query('SELECT open_ai_assistant_id FROM assistants WHERE id=$1', [assistantId]);
        if (assistantRow.rows.length === 0)
            return c.json({ error: 'Assistant not found' }, 404);
        const openAiAssistantId = assistantRow.rows[0].open_ai_assistant_id;
        const threadRow = await query('SELECT openai_threadid, assistant_id FROM threads WHERE id=$1', [threadId]);
        if (threadRow.rows.length === 0)
            return c.json({ error: 'Thread not found' }, 404);
        if (threadRow.rows[0].assistant_id !== assistantId)
            return c.json({ error: 'Thread does not belong to assistant' }, 400);
        ...
        return streamSSE(c, async (stream) => {
            let assistantText = '';
            try {
                const runStream = await openai.beta.threads.runs.stream(openAiThreadId, {
